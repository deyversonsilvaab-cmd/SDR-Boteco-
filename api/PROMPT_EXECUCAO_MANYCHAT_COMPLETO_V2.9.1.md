# Prompt de execução — ManyChat completo — SDR Sr. Boteco v2.9.1

> Objetivo: alinhar o ManyChat ao webhook v2.9.1 sem usar a IA paga do ManyChat, sem alterar o `WEBHOOK_SECRET` e sem criar automações concorrentes. Esta versão corrige a avaliação 1–5, adiciona CTA de avaliação Google para nota 5 e deixa as URLs de ação escondidas em botões no Instagram.
...
3. Se a versão não for 2.9.1, não alterar os fluxos ainda. Publicar primeiro o ZIP v2.9.1.
...
O webhook v2.9.1 já devolve a intenção e os CTAs corretos. O fluxo do ManyChat deve apenas renderizar o `ai_reply` e os botões indicados pelos campos CTA.
...
6. Para delivery, a v2.9.1 devolve normalmente 3 botões: **Pedido direto**, **iFood**, **99Food**.
...
## Nota específica v2.9.1 — fallbacks do Instagram
