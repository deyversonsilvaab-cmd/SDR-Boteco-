...
await test("health e payload reportam 2.9.6", async () => {
  const res = makeRes();
  await handler({ method: "GET", headers: {}, query: {} }, res);
  assert(res.payload?.version === "2.9.6", JSON.stringify(res.payload));
...
console.log("\nFallback Instagram v2.9.6: todos os testes passaram.");
